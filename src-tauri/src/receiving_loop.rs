...
use crate::{
    communication_manager::{CommunicationManager, CommunicationManagerState, DeviceName},
    ...
    use_state_in_mutex(
        ...
        &mut |communication_manager: &mut CommunicationManager| {
            ...
            for device in communication_manager.get_initialized_devices() {
                use_state_in_mutex(&log_state, &mut |log| match communication_manager.get_data(
                    device,
                    &mut result.parsed_packets,
                    log,
                ) {
                    Ok(_) => {}
                    Err(err) => {
                    ...
                    if let Some(new_ports) = output.new_available_port_names {
                        if !new_ports.is_empty() {
                            ...
                            use_state_in_mutex(
                                &app_handle.state::<CommunicationManagerState>(),
                                &mut |communication_manager| {
                                    let _ = communication_manager.init_device(
                                        &new_ports[0].name,
                                        57600,
                                        new_id,
                                    );
    ...
    fn can_receive_and_parse_data_with_multiple_rfds() {
        ...
        use_state_in_mutex(
            ...
            &mut |communication_manager| {
                ...
                communication_manager.ps_manager =
                    Arc::new(default_packet_structure_manager().into());
            },
        );
        //run the main receiving loop and print if any data is received
        loop {
                ...
                Ok(output) => {
                        ...
                        if !new_ports.is_empty() {
                            ...
                            use_state_in_mutex(
                                ...
                                &mut |communication_manager| {
                                    ...
                                    let _ =
                                        communication_manager.init_device("COM14", 57600, new_id_2);
                                },
                            );
                        }
                    }
                    if !output.parsed_packets.is_empty() {
                        for packet in output.parsed_packets {
                            println!("{:?} {}", packet.field_data[0], packet.structure_id);
                            //print timestamp and packet structure id
                        }
                    }
                }
                Err(err) => {
