...
use crate::{communication_manager::CommunicationManager, state::mutex_utils::use_state_in_mutex};
pub struct CommunicationManagerState {
    communication_manager: Mutex<CommunicationManager>
}

pub fn use_communication_manager<ReturnType>(
    communication_manager_state: tauri::State<'_, CommunicationManagerState>,
    callback: &mut dyn FnMut(&mut CommunicationManager) -> Result<ReturnType, anyhow::Error>,
