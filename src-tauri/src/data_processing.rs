...
impl DataProcessor {
    ...
    pub fn generate_display_field_names(
        &mut self,
        packet_structure_manager: &PacketStructureManager,
    ) -> Result<&Vec<DisplayPacketFieldNames>, (&Vec<DisplayPacketFieldNames>, String)> {
        self.name_list = vec![];
        for i in 0..packet_structure_manager.packet_structures.len() {
            self.name_list.push(DisplayPacketFieldNames::default());
            self.name_list[i]
                .field_names
                .push("Time Received".to_owned());
            for j in 0..packet_structure_manager.packet_structures[i].fields.len() {
                self.name_list[i].field_names.push(
                    packet_structure_manager.packet_structures[i].fields[j]
                        .name
                        .clone(),
                );
            }
