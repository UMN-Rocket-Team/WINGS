...
impl CommsIF for TeleDongleAdapter {
    ...
    fn init_device(&mut self, port_name: &str, _baud: u32) -> anyhow::Result<()> {
        if port_name.is_empty() {
            ...
            self.parse_device_data(&mut vec![],&mut vec![])?;
            self.write_port(&[0x6D,0x20,0x32,0x30,0x0A,0x6D,0x20,0x30,0x0A,0x63,0x20,0x46,0x20,0x34,0x33,0x35,0x30,0x35,0x30,0x0A,0x6D,0x20,0x32,0x30,0x0A,0x6D,0x20,0x30,0x0A,0x6D,0x20,0x32,0x30,0x0A,0x6D,0x20,0x30,0x0A,0x63,0x20,0x54,0x20,0x30,0x0A,0x6D,0x20,0x32,0x30,0x0A,0x6D,0x20,0x32,0x30,0x0A])?;
        }
        Ok(())
    }
