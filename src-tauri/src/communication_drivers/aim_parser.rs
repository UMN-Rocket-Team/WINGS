    ...
    pub fn default(ps_manager: &mut PacketStructureManager) -> AimParser {
        ...
        AimParser {
            ...
            packet_ids: PacketIdList {
                ...
                line_a: ps_manager.enforce_packet_fields(
                    LINE_A,
                    vec!["System time", "Delta time", "ADC", "Is_On", "Is_Input"],
                ),
                line_b: ps_manager.enforce_packet_fields(
                    LINE_B,
                    vec!["System time", "Delta time", "ADC", "Is_On", "Is_Input"],
                ),
                line_c: ps_manager.enforce_packet_fields(
                    LINE_C,
                    vec!["System time", "Delta time", "ADC", "Is_On", "Is_Input"],
                ),
                line_d: ps_manager.enforce_packet_fields(
                    LINE_D,
                    vec!["System time", "Delta time", "ADC", "Is_On", "Is_Input"],
                ),
                ...
                mag: ps_manager.enforce_packet_fields(
                    MAG,
                    vec!["System time", "Delta time", "X flux", "Y flux", "Z flux"],
                ),
                gps: ps_manager.enforce_packet_fields(
                    GPS,
                    vec![
                        "System time",
                        "Delta time",
                ...
                status: ps_manager.enforce_packet_fields(
                    STATUS,
                    vec![
                        "System time",
                        "Delta time",
                ...
                identifier: ps_manager.enforce_packet_fields(
                    IDENTIFIER,
                    vec!["System time", "Delta time", "Identifier"],
                ),
                gps_time: ps_manager.enforce_packet_fields(
                    GPS_TIME,
                    vec![
                        "System time",
                        "Delta time",
                ...
                timestamp: ps_manager.enforce_packet_fields(
                    TIMESTAMP,
        ...
        while i < length.into() {
            ...
            match delimiter {
                ...
                0x12 => {
                    ...
                    data.push(
                        PacketFieldType::UnsignedByte.parse(&[transmission[i + 8] & 0b00111111])?,
                    );
    ...
    fn test_parse_from_json() {
        ...
        for value in json_string_array {
            if let serde_json::Value::String(str) = value {
                ...
                let mut ans = vec![];
                aim.parse_transmission(&mut byte_array, &mut ans)
                    .expect("parser");
                collector.push(ans);
            }
        }
