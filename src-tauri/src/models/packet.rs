...
impl PacketFieldValue {
    ...
    pub fn to_le_bytes(&self, field_type: PacketFieldType) -> anyhow::Result<Vec<u8>> {
        ...
        match self {
            ...
            PacketFieldValue::Number(i) => match field_type {
                ...
                _ => Err(anyhow::anyhow!(
                    "Number value being processed as String or bool"
                )),
            },
        }
    }
...
impl PacketFieldType {
    ...
    pub fn parse(&self, bytes: &[u8]) -> anyhow::Result<PacketFieldValue> {
        match self {
            PacketFieldType::UnsignedByte => Ok(PacketFieldValue::Number(
                u8::from_le_bytes(slice_to_fixed_size::<1>(bytes)).into(),
            )),
            PacketFieldType::SignedByte => Ok(PacketFieldValue::Number(
                i8::from_le_bytes(slice_to_fixed_size::<1>(bytes)).into(),
            )),
            PacketFieldType::UnsignedShort => Ok(PacketFieldValue::Number(
                u16::from_le_bytes(slice_to_fixed_size::<2>(bytes)).into(),
            )),
            PacketFieldType::SignedShort => Ok(PacketFieldValue::Number(
                i16::from_le_bytes(slice_to_fixed_size::<2>(bytes)).into(),
            )),
            PacketFieldType::UnsignedInteger => Ok(PacketFieldValue::Number(
                u32::from_le_bytes(slice_to_fixed_size::<4>(bytes)).into(),
            )),
            PacketFieldType::SignedInteger => Ok(PacketFieldValue::Number(
                i32::from_le_bytes(slice_to_fixed_size::<4>(bytes)).into(),
            )),
            PacketFieldType::UnsignedLong => Ok(PacketFieldValue::Number(u64::from_le_bytes(
                slice_to_fixed_size::<8>(bytes),
            ) as f64)),
            PacketFieldType::SignedLong => Ok(PacketFieldValue::Number(i64::from_le_bytes(
                slice_to_fixed_size::<8>(bytes),
            ) as f64)),
            PacketFieldType::Float => Ok(PacketFieldValue::Number(
                f32::from_le_bytes(slice_to_fixed_size::<4>(bytes)).into(),
            )),
            PacketFieldType::Double => Ok(PacketFieldValue::Number(f64::from_le_bytes(
                slice_to_fixed_size::<8>(bytes),
            ))),
            PacketFieldType::UnsignedTwoFour => {
                let mut raw: u32 = 0;
            ...
            PacketFieldType::String => {
                Ok(PacketFieldValue::String(String::from_utf8(bytes.to_vec())?))
            }
    ...
    pub fn make_from_string(&self, input: &str) -> anyhow::Result<PacketFieldValue> {
        Ok(match self {
            ...
            PacketFieldType::Bool => PacketFieldValue::Bool(
                input.to_lowercase() == "true" || input.to_lowercase() == "t",
            ),
    ...
    pub fn size(&self) -> anyhow::Result<usize> {
        match self {
            ...
            PacketFieldType::String => Err(anyhow::anyhow!(
                "size of string is unknown from this context"
            )),
