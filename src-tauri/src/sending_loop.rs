...
use crate::{
    ...
    file_handling::{
        config_struct::ConfigState,
        log_handlers::{FileHandlingState, LogHandler},
    },
    packet_generator::generate_packet,
...
impl BackgroundTask {
    ...
    pub fn run_repeatedly<F>(mut callback: F) -> BackgroundTask
    where
        F: FnMut(),
        F: Send + 'static,
    {
        ...
        thread::spawn(move || {
            let is_stopped = || matches!(rx.try_recv(), Err(mpsc::TryRecvError::Disconnected));
...
impl SendingLoop {
    pub fn start(
        ...
        self.task = Some(BackgroundTask::run_repeatedly(move || {
            ...
            // ##########################
            match mode {
                SendingModes::FromCSV => match use_state_in_mutex(
                    &file_handling_state,
                    &mut |file_handler: &mut LogHandler| file_handler.read_packet(),
                ) {
                    Ok(packet) => {
                ...
                SendingModes::TimeStampAndIncreasing => {
                    ...
                    for i in 1..packet_structure.fields.len() {
                        output_string
                            .push(((packets_sent.overflowing_add(i as u32).0) as u8).to_string());
                    }
                    packet_to_send = Some(StringRecord::from(output_string));
                }
            }
            ...
            // Send Packet
            // ##########################
            match use_state_in_mutex(
                &app_handle.state::<Mutex<CommunicationManager>>(),
