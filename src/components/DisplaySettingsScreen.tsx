...
const RecursiveFlexviewEditor = (props: {
    ...
    createEffect(() => {
        ...
        if (
            loadedDisplayData.loadedDisplays.length > 0 ||
    ...
    const currentObject = createMemo(() => flexviewObjects[props.objectIndex]);

    return (
        <Show when={currentObject()} fallback={<div>Unknown flexview object</div>}>
            {(obj) => (
                <Switch>
                    <Match when={obj().type === "display"}>
                        {(() => {
                            const display = obj() as FlexviewDisplay;
                            return (
                                <div class="flex items-center justify-center p-2 flex-grow">
                                    <h1>{displays[display.index]?.displayName}</h1>
                                </div>
                            );
                        })()}
                    </Match>

                    <Match when={obj().type === "layout"}>
                        {(() => {
                            const layout = () => obj() as FlexviewLayout;
                            ...
                            return (
                                <div class="flex-grow flex flex-col p-2 gap-2">
                                    <div class="flex items-center justify-start border-2 border-gray-700 dark:border-gray-300 w-full overflow-auto p-2">
                                        {/*Element Buttons*/}
                                        <For each={Array.from(displayRegistry.values())}>
                                            {(typeDef) => (
                                                <button
                                                    type="button"
                                                    class="m-1 text-black bg-gray-300 hover:bg-slate-300 focus:outline-none focus:ring-4
                                    ...
                                    >
                                        <Show when={layout().children.length > 0} fallback={<p>Empty layout</p>}>
                                            <For each={layout().children}>
                                                {(TotalArrayObjectIndex, childArrayObjectIndex) => {
                                                    const weight_calc = () =>
                                                        `${(layout().weights[childArrayObjectIndex()] / totalWeight()) * 100}%`;
                                                    ...
                                                    return (
                                                        ...
                                                        >
                                                            <div class="flex overflow-auto">
                                                                <button
                                                                    type="button"
                                                                    class="m-5 z-[1] cursor-pointer p-0 bg-transparent border-0"
                                                                    aria-label="Remove"
                                                                    onClick={() => {
                                                                ...
                                                                >
                                                                    <img
                                                                        ...
                                                                        src={closeIcon}
                                                                        draggable={false}
                                                                        class="w-[25px] dark:invert"
                                                                    />
                                                                </button>

                                                                <Show when={layout().weights.length > 1}>
                                                                    <input
                                                                        class="w-[50px] m-5 bg-transparent border-b-2 border-black"
                                                                        ...
                                                                        onChange={(event) => {
                                                                            ...
                                                                            setFlexviewObjects(props.objectIndex, {
                                                                                ...
                                                                                weights: layout().weights.toSpliced(
                                                                                    childArrayObjectIndex(),
                                                                                    1,
                                                                ...
                                                                <Show when={flexviewObjects[TotalArrayObjectIndex]?.type == "display"}>
                                                                    <button
                                                                        ...
                                                                        onClick={() => {
                                                                            const childDisplay =
                                                                                flexviewObjects[TotalArrayObjectIndex] as FlexviewDisplay;
                                                                            showModal(
                                                                                displayRegistry.get(displays[childDisplay.index]!.type)!
                                                                                    .settingsModal ?? 0,
                                                                                {
                                                            ...
                                                            <RecursiveFlexviewEditor objectIndex={TotalArrayObjectIndex} />
                                                        </div>
                                                    );
                                                }}
                                            </For>
