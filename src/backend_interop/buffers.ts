...
export const unDecimatedPackets: Record<number, PacketData[]> = [];
...
export const pushParsedPackets = (packets: Packet[]): void => {
    ...
    for (const structureId in sortedNewParsedPackets) {
        ...
        if (decVars[+structureId].ptr2 + sortedNewParsedPackets[+structureId].length > (2 * decVars[+structureId].wall)) {
            ...
            unDecimatedPackets[+structureId].push(...sortedNewParsedPackets[structureId]); // For Readout.tsx
        }
        // console.log(parsedPackets[+structureId].length)
        decVars[+structureId].ptr2 = parsedPackets[+structureId].length; // ptr2 should basically always be the same as 2wall once we reach max capacity, but good to have this anyways
